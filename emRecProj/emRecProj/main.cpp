...
//
///** Global variables */
String face_cascade_name = "/Users/shobhikapanda/Downloads/opencv-2.4.10/data/haarcascades/haarcascade_frontalface_alt.xml";
String eyes_cascade_name  = "/Users/shobhikapanda/Downloads/opencv-2.4.10/data/haarcascades/haarcascade_eye.xml";
...
void ASMpoints(const char* filename){
   ...
   // path = "/Users/shobhikapanda/Documents/data/testface.jpg";
    ...
    if (!stasm_search_single(&foundface, landmarks,
                             (char*)frame.data, frame.cols, frame.rows,filename, "/Users/shobhikapanda/Downloads/testing"))
    {
    ...
    else
    {
      ...
      //  float outputAnger = svmclassifier.predictionAnger(queryFeature);
        cout<<outputHappy << "happy" <<"\n";
        cout<<outputSurprised << "surprised" << "\n";
      //  cout<<outputDisgust << "Disgust" << "\n";
     //   cout<<outputAnger << "Anger" << "\n";
        ostringstream diffValue;
        diffValue <<  diff << " , " << changed;
        if (changed > overallFeatureChange) {
            diffValue <<  "  At :: Emotion ";
        } else {
            diffValue <<  " At :: Neutral ";
        }
        
        putText(frame, diffValue.str().c_str(), Point(100, 100), CV_FONT_NORMAL, 0.5, Scalar(0,0,0), 2);
        
        ostringstream predictionInformation;
        predictionInformation <<  " Result :: " ;
        if (outputHappy != 0)
        ...
        putText(frame, predictionInformation.str().c_str(), Point(100, 150), CV_FONT_NORMAL, 0.5, Scalar(0,0,0), 2);
